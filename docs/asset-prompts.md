...
> **組込みの土台はできています**：`packages/client/public/` を新設し、SNS共有カード `og.svg` とファビコン `favicon.svg` を配置・`index.html` から参照済み（暫定SVG）。
> ベストには **PNG/JPGのラスター** が必要なので、§2 の hero / 背景を生成して `public/og.png`（1200×630）に置き換え、`index.html` の `og:image`/`twitter:image` をそれに差し替えてください。
> カード17種は §1 のプロンプトで生成し `public/cards/<key>.png` に配置。組込みは `CARD_INFO` の `icon` を画像パスにフォールバック付きで差し替えるだけ（現状は絵文字でも成立）。
