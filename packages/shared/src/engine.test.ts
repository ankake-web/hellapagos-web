...
import { CANNIBAL_FOOD_PER_BODY, COCONUT_WATER, DECK_COMPOSITION, DECK_TOTAL } from './deckConfig.js';
import {
  aliveCount,
  canEscapeAll,
  castVote,
...
test('投票中に資源カードを自分へ使うと、消費して自分だけ追放を免れる', () => {
  let s = mk(3, 8);
  s.players.forEach((p) => (p.hand = []));
  s.phase = 'vote';
  s.voteReason = 'food';
  s.pendingEliminations = 1;
  s.players[1].hand = [card('s1', 'sardine_can')];
  // p1 が自分にイワシ缶（食料）を使って身を守る → カード消費＆voteSafe
  s = playCard(s, 'p1', 's1');
  const p1 = s.players.find((p) => p.id === 'p1')!;
  assert.equal(p1.voteSafe, true);
  assert.equal(p1.hand.length, 0, 'カードは消費される');
  // 全員が p1 に投票しても追放されない（手札0でも voteSafe で守られる）
  s.players.forEach((p) => (p.vote = undefined));
  s = castVote(s, 'p0', 'p1');
  s = castVote(s, 'p2', 'p1');
  s = castVote(s, 'p1', 'p0');
  s = resolveVote(s);
  assert.equal(s.players.find((p) => p.id === 'p1')!.alive, true, '自己救済で生存');
});

test('投票理由に合わない資源は自己救済に使えない（水不足に食料カード）', () => {
  let s = mk(3, 8);
  s.players.forEach((p) => (p.hand = []));
  s.phase = 'vote';
  s.voteReason = 'water';
  s.pendingEliminations = 1;
  s.players[1].hand = [card('s1', 'sandwich')];
  const before = JSON.stringify(s.players[1].hand);
  s = playCard(s, 'p1', 's1'); // 水不足にサンドイッチ→無効
  assert.equal(JSON.stringify(s.players.find((p) => p.id === 'p1')!.hand), before, '無効なので消費されない');
  assert.notEqual(s.players.find((p) => p.id === 'p1')!.voteSafe, true);
});

test('無用品は使用不可（手札に残り続ける）', () => {
  let s = mk(3, 5);
  const a = actor(s);
  a.hand = [card('j1', 'junk')];
  s = playCard(s, a.id, 'j1');
  assert.equal(s.players.find((p) => p.id === a.id)!.hand.length, 1, '無用品は使えず手札に残る');
});
...
// ---- 永続カードは「使って初めて」効果＆公開 ----
test('永続カードは使って初めて公開され、効果が出る（斧）', () => {
  let s = mk(3, 5);
  const a = actor(s);
  a.hand.push(card('ax1', 'axe'));
  const otherId = s.players.find((p) => p.id !== a.id)!.id;
  // 使用前：永続欄には出ない（他者にも本人にも）。本人の手札にはある。
  assert.equal(redactFor(s, otherId, a.id).players.find((p) => p.id === a.id)!.permanents.includes('axe'), false, '使用前は他者に非公開');
  const ownBefore = redactFor(s, a.id, a.id).players.find((p) => p.id === a.id)!;
  assert.equal(ownBefore.permanents.includes('axe'), false, '使用前は永続欄に出ない');
  assert.ok(ownBefore.hand?.some((c) => c.kind === 'axe'), '本人の手札にはある');
  // 斧を使用(発動) → 公開される
  s = playCard(s, a.id, 'ax1');
  assert.equal(redactFor(s, otherId, a.id).players.find((p) => p.id === a.id)!.permanents.includes('axe'), true, '使用後は他者にも公開');
  // 発動後の木集めは +2（基本1＋斧1）
  s.raftProgress = 0;
  s.raftSeats = 0;
  s = takeAction(s, a.id, 'wood', 0);
  assert.equal(s.raftProgress, 2, '発動後は木+2');
});
...
test('漂着物デッキは合計54枚', () => {
  ...
  assert.equal(DECK_TOTAL, 54);
});

test('ココナッツは水+COCONUT_WATER（プールに加わる）', () => {
  let s = mk(3, 5);
  const a = actor(s);
  a.hand = [card('co', 'coconut')];
  const before = s.water;
  s = playCard(s, a.id, 'co'); // 行動フェイズ → プールへ
  assert.equal(s.water, Math.min(RESOURCE_CAP, before + COCONUT_WATER));
});

test('ほら貝で進行中の追放を1回無効化（誰も脱落しない）', () => {
  let s = mk(3, 8);
  s.players.forEach((p) => (p.hand = []));
  s.phase = 'vote';
  s.voteReason = 'food';
  s.pendingEliminations = 1;
  s.players[1].hand = [card('h1', 'conch')];
  const beforeAlive = aliveCount(s);
  s = playCard(s, 'p1', 'h1');
  assert.equal(aliveCount(s), beforeAlive, '誰も脱落していない');
  assert.equal(s.players.find((p) => p.id === 'p1')!.hand.length, 0, 'ほら貝は消費');
});

test('棍棒装備で自分の票が2票になり、最多得票を作れる', () => {
  let s = mk(4, 6);
  s.players.forEach((p) => (p.hand = []));
  s.players[0].revealed = ['club']; // 装備済み
  s.players[2].sick = true; // 投票不可
  s.players[3].sick = true;
  s.phase = 'vote';
  s.voteReason = 'food';
  s.pendingEliminations = 1;
  s.players.forEach((p) => (p.vote = undefined));
  s = castVote(s, 'p0', 'p3'); // 棍棒で2票
  s = castVote(s, 'p1', 'p2'); // 1票
  assert.ok(isVoteReady(s));
  s = resolveVote(s);
  assert.equal(s.players.find((p) => p.id === 'p3')!.alive, false, '棍棒の2票で p3 が脱落');
});

test('トタン板は銃を1回だけ防ぐ（板は消費）', () => {
  let s = mk(4, 9);
  s.players.forEach((p) => (p.hand = []));
  s.players[0].hand = [card('g', 'gun'), card('b', 'bullet')];
  s.players[3].hand = [card('t', 'tin_sheet')];
  s.phase = 'action';
  const before = aliveCount(s);
  s = playCard(s, 'p0', 'g', 'p3');
  const p3 = s.players.find((p) => p.id === 'p3')!;
  assert.equal(p3.alive, true, 'トタン板で生存');
  assert.equal(p3.hand.some((c) => c.kind === 'tin_sheet'), false, '板は消費');
  assert.equal(aliveCount(s), before);
});

test('マッチで汚れた水を病気にならず飲める', () => {
  let s = mk(3, 5);
  const a = actor(s);
  a.hand = [card('m', 'matches'), card('dw', 'dirty_water')];
  s = playCard(s, a.id, 'm');
  s = playCard(s, a.id, 'dw');
  const a2 = s.players.find((p) => p.id === a.id)!;
  assert.equal(a2.sick, false, 'マッチで病気にならない');
  assert.notEqual(a2.matchSafe, true, 'マッチは消費');
});

test('人肉BBQは死体1体につき食料+CANNIBAL_FOOD_PER_BODY', () => {
  let s = mk(4, 9);
  s.players.forEach((p) => (p.hand = []));
  s.players[0].hand = [card('bbq', 'cannibal_bbq')];
  s.bodiesAvailable = 2;
  const before = s.food;
  s = playCard(s, 'p0', 'bbq');
  assert.equal(s.food, Math.min(RESOURCE_CAP, before + 2 * CANNIBAL_FOOD_PER_BODY));
  assert.equal(s.bodiesAvailable, 0, '死体は消費');
});

test('望遠鏡で相手の手札を覗ける（本人だけに届く）', () => {
  let s = mk(3, 5);
  const a = actor(s);
  const other = s.players.find((p) => p.id !== a.id)!;
  a.hand = [card('tel', 'telescope')];
  other.hand = [card('x1', 'sandwich'), card('x2', 'gun')];
  s = playCard(s, a.id, 'tel', other.id);
  assert.equal(s.lastPeek?.hand.length, 2);
  assert.ok(redactFor(s, a.id, a.id).peek, '本人には届く');
  assert.equal(redactFor(s, other.id, a.id).peek, undefined, '他者には届かない');
});

// ---- 既存効果の修正 ----
test('血清は毒を治すが、その回の木材は失う', () => {
  let s = mk(3, 5);
  const a = actor(s);
  a.hand = [card('se', 'serum')];
  a.sick = true;
  s.raftProgress = 3;
  s.raftSeats = 0;
  s.lastWoodGain = { playerId: a.id, amount: 3 };
  s = playCard(s, a.id, 'se');
  const a2 = s.players.find((p) => p.id === a.id)!;
  assert.equal(a2.sick, false, '毒が治る');
  assert.equal(s.raftProgress, 0, 'その回の木材を失う');
});

test('フルーツバスケットは生存チェック中だけ使える（行動中は不可）', () => {
  const s = mk(3, 5);
  const a = actor(s);
  a.hand = [card('fb', 'fruit_basket')];
  const r = playCard(s, a.id, 'fb'); // 行動フェイズ
  assert.equal(r, s, '行動フェイズでは使えず state 不変');
});
